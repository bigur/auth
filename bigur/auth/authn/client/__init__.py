...
async def authenticate_client(request: Request, params: MultiDict) -> None:
    ...
    if 'client_id' in params:
            ...
            if client.has_password():
                if 'client_secret' not in params:
                    raise HTTPUnauthorized(
                        reason='Client\'s credentials not specified.')
            else:
                if 'client_secret' not in params:
                    return client
        ...
        if not client.verify_password(params['client_secret']):
            raise HTTPForbidden(reason='Invalid client\'s password.')
    else:
        raise HTTPUnauthorized(reason='Parameter `client_id\' is not set.')
